...
def test_count_tokens():
    ...
    assert count_tokens(TEXT, interlab.lang_models.AnthropicModel(api_key="Foo")) == 57
    print(str(interlab.lang_models.OpenAiChatModel(model="gpt-4", api_key="Foo")))
    assert (
        count_tokens(
            TEXT, interlab.lang_models.OpenAiChatModel(model="gpt-4", api_key="Foo")
        )
        == 40
    )
    assert (
